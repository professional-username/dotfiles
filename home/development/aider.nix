{ inputs, lib, config, pkgs, ... }: {
  ...
  # Set the aider command to read from the config file in .config
  programs.fish.shellAliases.aider = "aider --config $HOME/.config/aider/.aider.conf.yml";

  # Create the config file
  home.file.".aider.conf.yml" = {
    target = ".config/aider/.aider.conf.yml";
    text = ''
        ##########################################################
        ...
        tool-warning-color: "${config.lib.stylix.colors.base09}"
        ...
        completion-menu-current-bg-color: "${config.lib.stylix.colors.base05}"
